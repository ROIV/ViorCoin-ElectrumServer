...
class BlockchainProcessor(Processor):
    ...
    def do_catch_up(self):
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("viorcoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def bitcoind(self, method, params=[]):
        ...
        while True:
            try:
                ...
                print_log("cannot reach viorcoind...")
                self.shared.pause()
                time.sleep(10)
    ...
    def getfullblock(self, block_hash):
        ...
        try:
            ...
            logger.error("viorcoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run viorcoind with txindex=1; use -reindex if needed.")
