...
The most up-to date version of this document is available at:
    ...
    https://github.com/ROIV/ViorCoin-ElectrumServer/blob/master/HOWTO.md
...
document, we assume that user is called 'viorcoin'). We also assume the
viorcoin user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run viorcoind and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit viorcoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running viorcoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `viorcoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser viorcoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - viorcoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/viorcoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download viorcoind

Older versions of Electrum used to require a patched version of viorcoind. 
This is not the case anymore since viorcoind supports the 'txindex' option.
We currently recommend viorcoind 1.0 stable.

If you prefer to compile viorcoind, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libminiupnpc-dev
    $ sudo su - viorcoin
    $ cd ~/src && git clone https://github.com/viorcoin/viorcoin.git
    $ cd viorcoin/src
    $ make -f makefile.unix
    $ strip viorcoind
    $ cp -a ~/src/viorcoin/src/viorcoind ~/bin/viorcoind

### Step 3. Configure and start viorcoind

In order to allow Electrum to "talk" to `viorcoind`, we need to set up an RPC
username and password for `viorcoind`. We will then start `viorcoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.viorcoin
    $ $EDITOR ~/.viorcoin/viorcoin.conf

Write this in `viorcoin.conf`:
...
If you have an existing installation of viorcoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ viorcoind -reindex

If you have a fresh copy of viorcoind start `viorcoind`:

    $ viorcoind

Allow some time to pass, so `viorcoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ viorcoind getinfo

Before starting the electrum server your viorcoind should have processed all
blockes and caught up to the current height of the network.
You should also set up your system to automatically start viorcoind at boot
time, running as the 'viorcoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/ROIV/ViorCoin-ElectrumServer.git
    $ cd ViorCoin-ElectrumServer
    $ sudo configure
...
and further instructions from the Electrum-VIOR full archival server foundry at:
http://foundry.electrum-vior.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-vior.conf) when starting up. This
file includes the database setup, viorcoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-vior.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-vior-server.
If you're using user viorcoin to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "viorcoin hard nofile 65536" >> /etc/security/limits.conf
     echo "viorcoin soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close viorcoind for incoming connections and connect outbound only

2. Consider restarting viorcoind (together with electrum-vior-server) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_vior_server" as the
unprivileged user.
...
If your system supports it, you may add electrum-vior-server to the /etc/init.d directory. 
...
viorcoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-vior and we'll try to congratulate you
on supporting the community by running an Electrum-VIOR node.

If you're operating a public Electrum-VIOR server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/electrum-vior-server
It'll contain announcements about important updates to Electrum-VIOR
